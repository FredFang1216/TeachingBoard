...
${students.map((s: any, index: number) => 
  `${index + 1}. ${s.name} - 积分：${s.score}分${s.height ? `，身高：${s.height}cm` : ''}${s.weight ? `，体重：${s.weight}kg` : ''}${s.heartRate ? `，心率：${s.heartRate}bpm` : ''}`
).join('\n')}

请基于以上数据生成一份专业的Markdown格式分析报告，必须使用提供的具体数据，包含以下部分：

1. **班级整体表现概览** - 使用提供的积分数据计算总人数、平均分、最高分、最低分等统计信息
2. **学生表现等级分布** - 根据提供的积分数据按区间分类（优秀180分以上、良好120-179分、一般80-119分、待提升80分以下）
3. **身体指标分析** - 使用提供的身高、体重、心率数据进行统计分析
4. **重点关注学生** - 根据积分数据找出表现较差需要关注的学生
5. **教学建议与改进措施** - 基于实际数据分析的具体建议
6. **下阶段目标设定** - 可量化的改进目标

重要要求：
- 必须使用提供的具体数据，不要显示undefined
- 使用Markdown格式
- 语言专业但易懂
- 数据准确，分析深入
- 建议具体可操作
